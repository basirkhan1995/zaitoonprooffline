...
class AccountStatementExcelService {

  static Future<void> exportToExcel({
    required AccountStatementModel accountStatement,
    ...
    final records = accountStatement.records;
    ...
    try {
      ...
      titleRange.merge();
      titleRange.setText("ACCOUNT STATEMENT");
      ...
      accNameRange.merge();
      accNameRange.setText("Account: ${accountStatement.accNumber} - ${accountStatement.accName ?? ''}");
      ...
      // Signatory
      if (accountStatement.signatory != null && accountStatement.signatory!.isNotEmpty) {
        final signatoryRange = sheet.getRangeByName('A$currentRow:G$currentRow');
        signatoryRange.merge();
        signatoryRange.setText("Signatory: ${accountStatement.signatory}");
        signatoryRange.cellStyle
          ..fontSize = 11
          ..hAlign = HAlignType.left;
        currentRow++;
      }
      ...
      // Contact Information
      String contactInfo = "";
      if (accountStatement.perPhone != null && accountStatement.perPhone!.isNotEmpty) {
        contactInfo += "Phone: ${accountStatement.perPhone}";
      }
      if (accountStatement.perEmail != null && accountStatement.perEmail!.isNotEmpty) {
        contactInfo += "${contactInfo.isNotEmpty ? " | " : ""}Email: ${accountStatement.perEmail}";
      }
      if (contactInfo.isNotEmpty) {
        final contactRange = sheet.getRangeByName('A$currentRow:G$currentRow');
        contactRange.merge();
      ...
      // Address
      if (accountStatement.address != null && accountStatement.address!.isNotEmpty) {
        final addressRange = sheet.getRangeByName('A$currentRow:G$currentRow');
        addressRange.merge();
        addressRange.setText("Address: ${accountStatement.address}");
      ...
      currencyRange.merge();
      currencyRange.setText("Currency: ${accountStatement.actCurrency ?? ''} (${accountStatement.ccySymbol ?? ''})");
      ...
      /// BALANCE INFORMATION
      // Credit Limit
      if (accountStatement.actCreditLimit != null) {
        final creditLimitRange = sheet.getRangeByName('A$currentRow:G$currentRow');
        creditLimitRange.merge();
        creditLimitRange.setText("Credit Limit: ${accountStatement.ccySymbol ?? ''}${accountStatement.actCreditLimit?.toAmount() ?? '0'}");
        creditLimitRange.cellStyle
          ..fontSize = 11
          ..hAlign = HAlignType.left;
        currentRow++;
      }
      ...
      double curBalance = double.tryParse(accountStatement.curBalance ?? '0') ?? 0;
      curBalanceRange.setText("Current Balance: ${accountStatement.ccySymbol ?? ''}${curBalance.toAmount()}");
      ...
      double avilBalance = double.tryParse(accountStatement.avilBalance ?? '0') ?? 0;
      avilBalanceRange.setText("Available Balance: ${accountStatement.ccySymbol ?? ''}${avilBalance.toAmount()}");
      ...
      summaryRow("TOTAL DEBIT:", "${accountStatement.ccySymbol ?? ''}${totalDebit.toAmount()}");
      summaryRow("TOTAL CREDIT:", "${accountStatement.ccySymbol ?? ''}${totalCredit.toAmount()}");

      double netBalance = totalCredit - totalDebit;
      String balanceColor = netBalance < 0 ? '#FF0000' : '#008000';
      summaryRow(
          "NET BALANCE:",
          "${accountStatement.ccySymbol ?? ''}${netBalance.toAmount()}",
